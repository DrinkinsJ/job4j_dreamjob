...
public class VacancyControllerTest {
    ...
    @Test
    public void whenGetVacancyThenReturnVacancyAndRedirectToVacancyPage() {
        var vacancy = new Vacancy(1, "testName", "desc", now(), true, 1, 2);
    ...
    @Test
    public void whenUpdateVacancyWithFileThenVacancyUpdatedAndRedirectToVacanciesPage() throws IOException {
        var vacancy = new Vacancy(1, "testName", "desc", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoCaptor = ArgumentCaptor.forClass(FileDto.class);
        when(vacancyService.update(vacancyArgumentCaptor.capture(), fileDtoCaptor.capture())).thenReturn(true);
        var model = new ConcurrentModel();
        var view = vacancyController.update(vacancy, testFile, model);
        var updateVacancy = vacancyArgumentCaptor.getValue();
        var actualFileDto = fileDtoCaptor.getValue();
        assertThat(vacancy).isEqualTo(updateVacancy);
        assertThat(fileDto).usingRecursiveComparison().isEqualTo(actualFileDto);
        assertThat(view).isEqualTo("redirect:/vacancies");
    }
    ...
    public void whenDeleteVacancyThenRedirectToVacanciesPage() {
        ...
        var model = new ConcurrentModel();
        var view = vacancyController.delete(model, 123);
        var deletedVacancy = model.getAttribute("vacancy");
        assertThat(deletedVacancy).isNull();
        assertThat(view).isEqualTo("redirect:/vacancies");
    }
}